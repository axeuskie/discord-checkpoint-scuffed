...
while year != "q" and year != "no":
    ...
    for result in results_dm:
        print(f"{result[1]} | {result[0]} messages")
    print("--- STATS ---")
    print(f"You have sent {totalMsg} messages in total... Incredible!")
    print(
        f"Your most active server was {results_servers[0][1]} with {results_servers[0][0]} messages in total..."
    )
    print(
        f"You sent the most messages to {results_dm[0][1]} with {results_dm[0][0]} messages in total..."
    )
    print("---------")
    year = input("Wanna try again ? \nq / no to quit\t")
